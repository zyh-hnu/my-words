...
def get_local_file_with_today(filename: str) -> Optional[str]:
    """从本地读取今天的文件内容"""
    newsletter_dir = get_newsletter_directory()
    today_formatted = current_date_formatted()
    file_path = os.path.join(newsletter_dir, today_formatted, filename)
    ...
    if os.path.exists(file_path):
        try:
            ...
            logger.error(f"读取本地文件 {file_path} 失败: {e}")
            return None
    return None


def put_local_file_with_today(filename: str, content: str) -> bool:
    """将内容保存到本地今天的文件"""
    newsletter_dir = get_newsletter_directory()
    today_formatted = current_date_formatted()
    ...
    try:
        ...
        logger.error(f"保存文件到本地失败 {file_path}: {e}")
        return False


def get_newsletter_directory() -> str:
    """获取 newsletter 的目录"""
    return config.settings.newsletter_dir


if __name__ == "__main__":
    print(current_date_formatted())
    print(yesterday_date_formatted())
